...
def viewimage(im,normalise=True,MINI=0.0, MAXI=255.0):
    ...
    if platform.system()=='Darwin': #on est sous mac
        ...
        prephrase='start /B "D:/GIMP/bin/gimp-2.10.exe" '#Remplacer D:/... par le chemin de votre GIMP
        endphrase= ''
    else:
...
def var_image(im,x0,y0,x1,y1):
    im_noise_filter_med = median_filter(im_noise)
    viewimage(im_noise_filter_med)
...
plt.show()im_noise_filter_med = median_filter(im_noise)
viewimage(im_noise_filter_med)
...
viewimage(imequal, get_gau_ker(2))
...
viewimage(im)
im_filter_lin = filtre_lineaire(im, get_gau_ker(2))
viewimage(im_filter_lin)
im_defilter = filtre_inverse(im_filter_lin, get_gau_ker(2))
viewimage(im_defilter)
